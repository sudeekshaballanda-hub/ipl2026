/*The advantage of storing array of structures as binary files is that we can use fseek to go to specific 
record and get the record.
...
int main()
{
    ...
    FILE *fp;
    FILE *fp1;
    printf("Enter the number number of records you want to store: ");
    scanf("%d",&n);
    data(n,e);
    write_to_file(n,e);
